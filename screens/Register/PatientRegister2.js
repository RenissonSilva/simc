...
static navigationOptions = {
  ...
  render() {
    return (
        <ScrollView style={styles.scrollView}>
          <Formik
            initialValues={{cep: '', rua: '', cidade: '', estado: '', pais: '', numero: ''}}
            onSubmit={ values => { this.Register(values)}}
            validationSchema={yup.object().shape({
              cep: yup
              .string()
              .required('Cep e um campo obrigatório'),
        ...
        {({ values, handleChange, errors, setFieldTouched, touched, isValid, handleSubmit }) => (
            <Fragment>
              ...
              <TextInputMask
                ...
                value={values.cep}
                onBlur= {() => {setFieldTouched('cep'); this.procurar_cep(values.cep) } }
                maxLength={9}
              ...
              { touched.estado && errors.estado &&
                <Text style={{fontSize:10, color: 'red'}}>{errors.estado}</Text>
              }
  ...
  Register(values){
    ...
    console.log( moment(params.values.idade,['DD-MM-YYYY','YYYY-MM-DD']).utc().format('YYYY-MM-DD'));
    ...
    if(this.state.cepunmasked){
      if(params){
          ...
          birthday: moment(params.values.idade,['DD-MM-YYYY','YYYY-MM-DD']).utc().format('YYYY-MM-DD'),
